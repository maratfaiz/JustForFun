...
| Папка | Что это | |
|---|---|---|
| `assets/icons-phosphor/` | **Phosphor Icons**, вес Bold — готовый набор, лицензия MIT | ← рекомендую |
...
Две иконки в наборе Phosphor составные, потому что таких в нём нет:
`icon-lumen` (кольцо + звезда) и `icon-critic-voice` (пузырь +
восклицательный знак). Собраны из его же деталей, в той же толщине линии.
...
node tools/phosphor_render.js package/assets /tmp/raw 1024
...
набора — правится одна строчка в этом файле, и всё пересобирается.
Чтобы перейти на другой вес (`regular`, `fill`, `duotone`) — меняется
константа `WEIGHT` в `tools/phosphor_render.js`.
